...
$user_id = $_SESSION['user_id']; // Landlord's user ID
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_all_read'])) {
    if (verify_csrf_token($_POST['csrf_token'])) {
        if (mark_all_notifications_as_read($pdo, $user_id)) {
            ...
            $_SESSION['error_flash'] = 'Could not mark all notifications as read.';
        }
    } else {
        $_SESSION['error_flash'] = 'Invalid request token.';
    }
    // Redirect back to the same page
    redirect('landlord/notifications.php');
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <?php if (!empty($all_notifications) && count(array_filter($all_notifications, fn($n) => !$n['is_read'])) > 0): ?>
        ...
        <div class="card-body p-0">
            <?php if (empty($all_notifications)): ?>
                <div class="text-center p-5">
                    ...
                    <h4 class="mt-3 text-muted">You have no notifications.</h4>
                    ...
                    <?php foreach ($all_notifications as $notif): ?>
                        <li class="list-group-item p-3 <?php echo $notif['is_read'] ? 'bg-light text-muted' : 'fw-bold'; ?>">
                            <div class="d-flex w-100 justify-content-between mb-1">
                                <h5 class="mb-0 text-<?php echo sanitize($notif['type']); ?>">
                                    <?php
                                    // Icon mapping
                                    $icon_map = [
                                    ...
                                    ?>
                                    <i class="bi <?php echo $icon_map[$notif['type']] ?? 'bi-bell-fill text-secondary'; ?> me-2"></i>
                                    <?php echo sanitize($notif['title']); ?>
                                </h5>
                                <small><?php echo date('M j, Y \a\t g:i a', strtotime($notif['created_at'])); ?></small>
                            </div>
                            <p class="mb-1 <?php echo $notif['is_read'] ? '' : 'text-dark'; ?>"><?php echo sanitize($notif['message']); ?></p>
                            <?php if ($notif['link']): ?>
